...
- **ما هو:** نظام رقمي يحوّل "الجمعية" التقليدية (تناوب مجموعة أشخاص على دفع مبلغ ثابت شهرياً واستلامه بالتناوب) من إدارة يدوية عبر واتساب وثقة شخصية، إلى منصة موثوقة بأربعة وكلاء ذكاء اصطناعي.
- **لمن:** مجموعات الأصدقاء والعائلات التي تدير جمعيات مالية اليوم، وبنك الإنماء كقناة اكتساب عملاء جديدة.
- **ما يميزه عن مجرد سجل دفعات رقمي:** تقييم مخاطر آلي لكل عضو جديد، ترتيب عادل لاستلام المبلغ، ووسيط تلقائي عند التأخر عن الدفع.
 
## المحتويات
 
- [الرابط التجريبي (Demo)](#الرابط-التجريبي-demo)
- [نظرة عامة](#نظرة-عامة)
- [المشكلة التي يحلها](#المشكلة-التي-يحلها)
- [ما الذي يميز جمعيتك؟](#ما-الذي-يميز-جمعيتك)
- [الوكلاء الأربعة](#الوكلاء-الأربعة)
...
| المعيار | الجمعية التقليدية (واتساب) | تطبيقات تتبع الجمعيات الحالية | جمعيتك |
|---|---|---|---|
| تقييم عضو جديد | حكم شخصي بدون أي أداة | غير موجود عادة | وكيل مخاطر آلي يطابق رقم الجوال ويرصد تضارب الاسم مع الرقم |
| ترتيب الاستلام | نقاش يدوي، غالباً حسب من يطلب أولاً | ترتيب ثابت أو عشوائي | وكيل دور يقترح ترتيباً بناءً على تاريخ الراتب والهدف المالي لكل عضو |
| متابعة التأخر | رسائل تذكير شخصية محرجة | تنبيه دفع بسيط بدون حل مقترح | وكيل وساطة يصيغ رسالة محايدة وخيارات إعادة جدولة تلقائياً |
| الشفافية | معرفة غير موثقة، تعتمد على ذاكرة منظّم الجمعية | سجل دفعات فقط | قاعدة بيانات مشتركة حقيقية + سجل زمني لكل قرار وكيل |
...
```mermaid
sequenceDiagram
    participant B as المتصفح (JS)
    participant F as Flask (agent_server.py)
    participant O as المنسّق (orchestrator.py)
    participant A as الوكيل (مثال: risk_agent.py)
    participant G as Groq (Llama 3.3 70B)
    participant D as SQLite

    B->>F: طلب HTTP (مثال: إضافة عضو)
    F->>O: handle_event() أو route_and_handle()
    O->>A: استدعاء الوكيل المناسب
    A->>G: call_llm_json()
    alt رد صالح
        G-->>A: حكم الوكيل (JSON)
    else فشل أو انتهاء مهلة
        A-->>A: تراجع لقيمة افتراضية محافظة
    end
    A->>D: تحديث حالة الدائرة/العضو
    A-->>F: النتيجة
    F-->>B: عرض الحكم داخل بطاقة الوكيل
```
 
مثال ملموس على التراجع: إن لم يرد Groq خلال المهلة المحددة أو أعاد JSON غير صالح، يُعيد `call_llm_json()` بعد إعادة محاولة واحدة حكماً افتراضياً محافظاً (مثل تصنيف "مخاطر متوسطة" بدل رفض الطلب بالكامل) — بحيث لا يتوقف تسجيل عضو جديد بسبب خطأ مؤقت من النموذج اللغوي.
...
هذا المشروع حالياً بمرحلة الهاكاثون، وتم تحديد نطاقه عمداً لإثبات فكرة الوكلاء الأربعة بدلاً من بناء بنية تحتية إنتاجية كاملة. الخطوات التالية مقسّمة حسب الأولوية والجهد المتوقع نحو منتج جاهز للإنتاج:
 
### المرحلة القادمة (جهد متوسط)
 
- **نظام مصادقة حقيقي**: الانتقال من كلمة المرور النصية المباشرة إلى تجزئة كلمات المرور، جلسات ورموز مصادقة، وتقييد لمعدل الطلبات، مع ربط فعلي بمنصة نفاذ للتحقق من الهوية بدلاً من المحاكاة الحالية.
- **قاعدة بيانات قابلة للتوسع**: الانتقال من SQLite إلى قاعدة بيانات إنتاجية (مثل PostgreSQL). الوصول لقاعدة البيانات معزول بالفعل داخل `accounts.py` و`circle_store.py` فقط، لذا الانتقال يطال هذين الملفين لا بنية المشروع كاملة.
- **فصل عرض نتائج الوكلاء عن الواجهة الرئيسية**: كل منطق الوكلاء الأربعة يعمل بالكامل على الخادم، ولا شيء منه في المتصفح. حالياً نتائجها تُعرض مباشرة داخل واجهة المنتج نفسها لأغراض العرض والتقييم في الهاكاثون. الخطوة القادمة نقل هذا العرض إلى لوحة مراقبة/تشغيل منفصلة، بحيث تبقى تجربة المستخدم النهائي نظيفة دون الحاجة لإظهار تفاصيل عمل كل وكيل بشكل مباشر.
 
### لاحقاً (جهد أكبر، يتطلب شراكات خارجية)
 
- **تكامل حقيقي مع الحسابات البنكية المفتوحة والدفع**: ربط فعلي بمزودي الدفع (مدى، Apple Pay، التحويل البنكي المباشر) بدلاً من محاكاة الدفع الحالية في الواجهة.
- **الترخيص التنظيمي**: دراسة متطلبات البنك المركزي السعودي الخاصة بتجميع واستثمار أموال الأعضاء عبر وكيل العائد، تمهيداً لتفعيل هذه الميزة ضمن إطار تنظيمي معتمد.
- **توسيع الشراكة مع بنك الإنماء**: تحويل الشراكة من فكرة عرض إلى قناة اكتساب عملاء فعلية، بحيث تصبح "جمعيتك" نقطة دخول حقيقية لعملاء جدد للبنك.
- **تطبيق جوال أصلي**: بناء نسخة أصلية لنظامي iOS وAndroid إلى جانب تجربة الويب الحالية القابلة للتثبيت كـ PWA.
---
